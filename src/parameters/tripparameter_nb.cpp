...
void init_TripParameters(nb::module_ &m)
{
    ...
    nb::class_<TripParameters, RouteParameters>(m, "TripParameters")
        ...
        .def("__init__", [](TripParameters *t, TripParameters::SourceType source, TripParameters::DestinationType destination, bool roundtrip, const bool steps, int number_of_alternatives, const std::vector<RouteParameters::AnnotationsType> &annotations, RouteParameters::GeometriesType geometries, RouteParameters::OverviewType overview, const std::optional<bool> continue_straight, std::vector<std::size_t> waypoints, std::vector<osrm::util::Coordinate> coordinates, std::vector<std::optional<osrm::engine::Hint>> hints, std::vector<std::optional<double>> radiuses, std::vector<std::optional<osrm::engine::Bearing>> bearings, const std::vector<std::optional<osrm::engine::Approach>> &approaches, bool generate_hints, std::vector<std::string> exclude, const BaseParameters::SnappingType snapping)
             {
                ...
                osrm_nb_util::assign_baseparameters(t,
                                                    ...
                                                    generate_hints,
                                                    exclude,
                                                    snapping); }, "source"_a = std::string(), "destination"_a = std::string(), "roundtrip"_a = true, "steps"_a = false, "alternatives"_a = 0, "annotations"_a = std::vector<std::string>(), "geometries"_a = std::string(), "overview"_a = std::string(), "continue_straight"_a = std::optional<bool>(), "waypoints"_a = std::vector<std::size_t>(), "coordinates"_a = std::vector<osrm::util::Coordinate>(), "hints"_a = std::vector<std::optional<osrm::engine::Hint>>(), "radiuses"_a = std::vector<std::optional<double>>(), "bearings"_a = std::vector<std::optional<osrm::engine::Bearing>>(), "approaches"_a = std::vector<std::string *>(), "generate_hints"_a = true, "exclude"_a = std::vector<std::string>(), "snapping"_a = std::string())
    ...
    nb::class_<TripParameters::SourceType>(m, "TripSourceType")
        .def("__init__", [](TripParameters::SourceType *t, const std::string &str)
            ...
            new (t) TripParameters::SourceType(source); }, "Instantiates a SourceType based on provided String value.")
        .def("__repr__", [](TripParameters::SourceType type)
             { return osrm_nb_util::enum_to_str(type, "TripSourceType", source_map); }, "Return a String based on SourceType value.");
    ...
    nb::class_<TripParameters::DestinationType>(m, "TripDestinationType")
        .def("__init__", [](TripParameters::DestinationType *t, const std::string &str)
            ...
            new (t) TripParameters::DestinationType(dest); }, "Instantiates a DestinationType based on provided String value.")
        .def("__repr__", [](TripParameters::DestinationType type)
             { return osrm_nb_util::enum_to_str(type, "TripDestinationType", dest_map); }, "Return a String based on DestinationType value.");
